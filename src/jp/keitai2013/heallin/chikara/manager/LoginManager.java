...
//		}
	...
	//友達になる
	public boolean followFrend(LoginInfo lf, String friend_id, boolean follow){
		
		if(CFConst.IS_MIRAI){
			return follow_frend_mirai(lf, friend_id, follow);
		}

		ArrayList<NameValuePair> params = new ArrayList<NameValuePair>();

		params.add(new BasicNameValuePair("sid", lf.sid.sid));
		params.add(new BasicNameValuePair("frend_id", ""+friend_id));
	...
	public boolean follow_frend_mirai(LoginInfo l, String friend_id , boolean follow){
		...
		if(follow){
			params.add(new BasicNameValuePair("addfriend", friend_id));
		}else{
			params.add(new BasicNameValuePair("deletefriend", friend_id));
		}
		...
		if(follow){
			rd = CFUtil.postData( "http://hellin.dip.jp/m/AddFriend.php", params, cookie);
		}else{
			rd = CFUtil.postData( "http://hellin.dip.jp/m/DeleteFriend.php", params, cookie);
		}
		...
		try {
			...
			String str_ok = rd.data;

			if(!str_ok.equalsIgnoreCase("OK")) return false;
