...
//			bmps[1] = ((BitmapDrawable) res.getDrawable(R.drawable.ic_launcher)).getBitmap();
			...
			for(int i=0;i<num;i++){
				...
				item.name = jc.getString("name");

//				if(!jc.isNull("user_name"))
//					item.name = jc.getString("user_name");
//				else item.name = null;
//
				item.icon_url = jc.getString("icon");
				//item.icon = PictureManager.downloadImageToBmp(item.icon_url, true);
				
				item.is_follow = true;

				//item.icon = CFUtil.getImage(jc.getString("icon"), 500, 500);
				//item.icon = CFUtil.getImage(jc.getString("icon"), 500, 500);
				//item.icon = PictureManager.downloadImageToBmp(url);
...
//			bmps[1] = ((BitmapDrawable) res.getDrawable(R.drawable.ic_launcher)).getBitmap();
	...
	public synchronized boolean search_mirai(LoginInfo l, String q){
		...
		rd = CFUtil.postData( "http://hellin.dip.jp/m/SearchFriend.php", params, cookie);
		...
		try {
			...
			JSONArray jdata = json.getJSONArray("searchresult");
			...
			for(int i=0;i<num;i++){
				...
				MItem item = new MItem();
				item.id = jc.getString("searchfriend");
