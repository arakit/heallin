...
public class TimeLineManager {
	...
	public interface OnUpdateTimelineImageListner{
...
//		public Date time_e;
	...
	public TimeLineManager(Context context, PictureManager pm){
		mContext = context;
		mPictureManager = pm;
	}
...
//	}
	...
	public synchronized boolean update(LoginInfo l, OnUpdateTimelineImageListner lis){

		if(CFConst.IS_MIRAI){
			return update_all_mirai(l, lis);
		}
...
//				item.picture = bmps[rand%2];
	...
	public synchronized boolean update_all_mirai(LoginInfo l,final OnUpdateTimelineImageListner lis){

		if( !update_mirai(l, 0, lis, true) ){
			return false;
		}

		while(true){
			int num1 = mList.size();
			if( !update_mirai(l, 1, lis, false) ){
				return false;
			}
			int num2 = mList.size();
			if(num1 == num2) break;
	...
	public synchronized boolean update_mirai(LoginInfo l,int reload,final OnUpdateTimelineImageListner lis, boolean clear){
		...
		rd = CFUtil.postData( "http://hellin.dip.jp/m/Timeline2.php", params, cookie);
...
//				else item.title = null
				...
				String user_image_url = item.user_icon_url = CFUtil.getStringN(jc, "timelineicon", null);
				...
				String image_url = item.image_url = CFUtil.getStringN(jc, "timelinepicture", null);

				//item.image = mPictureManager.getBitmap(image_url);

				if( item.image==null){
//					mPictureManager.requestDownloadBtmap(image_url, new GetImageTaskListener() {
//						@Override
//						public Bitmap onGetTask(PictureManager pm, String url) {
//							CFUtil.Log("onGetTask "+url);
//							return CFUtil.getImage(url, 250, 1000*10);
//						}
//						@Override
//						public void onUpdateImage(PictureManager pm, String url) {
//							CFUtil.Log("onUpdateImage "+url);
//							item.image = pm.getBitmap(url);
//							lis.onUpdate(item, url);
//						}
//					});
				}
...
//	}
	...
	public synchronized void sortByDate(final boolean asc){
